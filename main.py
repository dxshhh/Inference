...
class PCN(bp.DynamicalSystemNS):
    def __init__(self, neuron_size, tau, sigam_scale, activation=bm.relu, name=None, batch_size=1, m = 1, noise = 2, zero_lastpar = False):
        ...
        self.m = m
        self.noise = noise
        self.zero_lastpar = zero_lastpar
        # parameters
        self.par_list = bm.ListVar([])
        for i in range(self.layer - 1):
            self.par_list.append(self.rng.normal(0,1,size = (neuron_size[i + 1], neuron_size[i])))
            self.par_num += neuron_size[i + 1] * neuron_size[i]
        if self.zero_lastpar == False:
            self.par_list.append(self.rng.normal(0,1,size = (1, neuron_size[-1])))
            self.par_num += neuron_size[-1]
        else:
            self.par_list.append(bm.zeros((1,neuron_size[-1])))
    ...
    def reset_neuron(self, batch_size=1, input=None):
        ...
        if input != None:
            ...
            self.var_list[-1] = self.rng.normal(self.par_list[-1].squeeze(),self.sigma_scale[-1],size = (batch_size,self.neuron_size[-1]))
    ...
    def update(self, train, time):
        ...
        for i in range(self.layer - 1):
            self.e[i].value = (
                ...
                self.sigma_list[i]))
        self.e[-1].value = ((self.var_list[-1] - self.par_list[-1]) @ bm.linalg.inv(self.sigma_list[-1]))
        ...
        if not train:
            ...
            self.v_list[0].value = self.v_list[0] + (-self.m * self.v_list[0]) * dt + 2 * self.m * tmp_noise * bm.sqrt(dt)
    ...
    def generate(self, test_batchsize = 10):
        ...
        self.var_list[-1] = self.rng.normal(self.par_list[-1].squeeze(), self.tau[-1]*self.noise/2, size = (test_batchsize,self.neuron_size[-1]))
        for i in range(self.layer - 1, 0, -1):
            self.var_list[i-1] = self.activation(self.var_list[i]) @ self.par_list[i-1] + self.rng.normal(0, self.tau[i-1]*self.noise/2, (test_batchsize, self.neuron_size[i-1]))
